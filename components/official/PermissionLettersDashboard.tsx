...
import { collection, query, where, getDocs, doc, updateDoc, serverTimestamp, getDoc, Timestamp, orderBy } from 'firebase/firestore';
...
export const PermissionLettersDashboard: React.FC = () => {
    ...
    const fetchLetters = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // FIX: Added orderBy to sort letters by creation date.
            const q = query(
        ...
        return (
            <div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md text-black">
                                            ...
                                            <li key={rn} className="flex items-center justify-between p-1 rounded hover:bg-gray-100">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    {(() => {
                                                        ...
                                                        const myApprovalRole = getMyApprovalRole();
                                                        const canApprove = myApprovalRole && user?.officialRole === `${dept}_hod` && filter === 'pending' && letter.approvals[myApprovalRole] === 'pending';
                    ...
                    {filter === 'pending' && getMyApprovalRole() && letter.approvals[getMyApprovalRole()!] === 'pending' && (
